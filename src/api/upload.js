/**
 * Blackbook — Upload API
 * POST /api/upload
 * Body: multipart/form-data { pdf: File, account_id: string }
 *
 * Runs the full pipeline server-side:
 * PDF → Gemini extract → validate → Gemini categorize → Supabase insert
 *
...
// ── Prompts (inlined so Vercel doesn't need to read files at runtime) ─────────
...
- tx_prefix must be exactly one of: UPI, MB, IMPS, NEFT, NACH, PCD, ZING, OTHER
  ...
  - NEFT: starts with NEFT/ or FT- (HDFC fund transfers)
...
3. description contains "SALARY" and direction credit → category: "Salary"
4. MB: prefix with "RAKESH KUMAR PANDEY" or family name → category: "Family Support"
5. NACH prefix → check description; usually "Streaming" or "SaaS & Apps"
6. description contains "REFUND" or note contains "Refund" → category: "Refund"
7. UPI note starts with "CAB" → category: "Cab", merchant: null
8. UPI note starts with "AUTO" or "Rick" or "Ric" → category: "Auto & Rickshaw", merchant: null
9. UPI note starts with "FOOD" → category: "Dining & Cafe", merchant: null
10. UPI to individual name, amount < ₹500, no clear note → category: "Auto & Rickshaw"
11. UPI to individual name, amount ₹500–₹5000, no clear note → category: "Split & Settle"
12. Large NEFT/IMPS/FT credit (>₹5000), direction credit, description contains "SALARY" → category: "Salary"
...
[
  {
    ...
    "merchant_display_name": "<clean name or null>",
...
function extractUPIHandle(description) {
...
function extractUPIMerchantRaw(description) {
  if (!description) return null
  const m = description.match(/^UPI\/([^/]+)\//)
  return m ? m[1].trim() : null
}

function extractUPINote(description) {
  if (!description || !description.startsWith('UPI/')) return null
  const parts = description.split('/')
  return parts[parts.length - 1].trim() || null
}

// ── Validation (inline — no file dependency) ─────────────────────────────────
...
function validate(extracted) {
  ...
  if (!Array.isArray(transactions) || transactions.length === 0) {
    errors.push('MISSING: transactions array empty')
  ...
  transactions.forEach((tx, i) => {
    const loc = `Row ${i + 1}`
    if (tx.amount == null)        errors.push(`${loc}: amount is null`)
    if (!tx.txn_date)             errors.push(`${loc}: txn_date is null`)
    if (!tx.direction)            errors.push(`${loc}: direction is null`)
    if (tx.balance_after == null) errors.push(`${loc}: balance_after is null`)
    if (tx.direction && !['debit','credit'].includes(tx.direction))
      errors.push(`${loc}: invalid direction "${tx.direction}"`)
    if (tx.txn_date && !/^\d{4}-\d{2}-\d{2}$/.test(tx.txn_date))
      errors.push(`${loc}: txn_date not YYYY-MM-DD`)
    if (tx.amount != null && tx.direction) {
      if (tx.direction === 'credit') credits += tx.amount
      if (tx.direction === 'debit')  debits  += tx.amount
    }
    if (tx.amount > 50000) warnings.push(`${loc}: large transaction ₹${tx.amount}`)
  ...
  if (opening_balance != null && closing_balance != null && errors.length === 0) {
    ...
    if (diff > TOLERANCE)
      errors.push(`BALANCE MISMATCH: computed ${expected}, stated ${closing_balance}, diff ₹${diff}`)
  }
...
export default async function handler(req, res) {
  ...
  const form = new IncomingForm({ maxFileSize: 20 * 1024 * 1024 }) // 20MB max
  let fields, files
  try {
  ...
  const accountId = Array.isArray(fields.account_id) ? fields.account_id[0] : fields.account_id
  ...
  // Use SSE to stream step-by-step progress to the UI
  res.setHeader('Content-Type', 'text/event-stream')
  res.setHeader('Cache-Control', 'no-cache')
  res.setHeader('Connection', 'keep-alive')

  function send(step, status, data = {}) {
  ...
  try {
    ...
    let reviewCount = 0
    const txRows = extracted.transactions.map((tx, i) => {
